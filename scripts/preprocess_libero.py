...
def parse_args():
    ...
    if args.val_json == "annotations_eval.json":
        # Keep the default explicit.
        pass
    return args
...
def process_split(
    ...
    for entry in tqdm(selected, desc=f"libero/{split_name}"):
        try:
            ...
            if debug_err_printed < 3:
                debug_err_printed += 1
                print(f"[WARN] libero/{split_name} skip entry due to exception:")
